...
const names = [
  "Local.Live.Captions-0.1.15-1.x86_64.rpm",
  "Local.Live.Captions_0.1.15_amd64.AppImage",
  "Local.Live.Captions_0.1.15_amd64.deb",
  "Local.Live.Captions_0.1.15_universal.dmg",
  "Local.Live.Captions_0.1.15_x64-setup.exe",
  "Local.Live.Captions_0.1.15_x64_en-US.msi",
  "Local.Live.Captions_universal.app.tar.gz",
  "SHA256SUMS",
  "latest.json",
];

function publication(commit = repairedCommit, identity = { tag: "v0.1.15", commit: repairedCommit }) {
...
describe("published release contract", () => {
  ...
  it("rejects a package omitted from the manifest or checksums", () => {
    ...
    expect(publicationErrors(incomplete)).toEqual([
      "latest.json does not contain the published URL for Local.Live.Captions_0.1.15_amd64.AppImage.",
      "SHA256SUMS does not cover Local.Live.Captions_0.1.15_amd64.AppImage.",
    ]);
  });
});
