...
const names = [
  "Local.Live.Captions-0.1.19-1.x86_64.rpm",
  "Local.Live.Captions_0.1.19_amd64.AppImage",
  "Local.Live.Captions_0.1.19_amd64.deb",
  "Local.Live.Captions_0.1.19_universal.dmg",
  "Local.Live.Captions_0.1.19_x64-setup.exe",
  "Local.Live.Captions_0.1.19_x64_en-US.msi",
  "Local.Live.Captions_universal.app.tar.gz",
  "SHA256SUMS",
  "latest.json",
];

function publication(commit = repairedCommit, identity = { tag: "v0.1.19", commit: repairedCommit }) {
...
describe("published release contract", () => {
  ...
  it("regresses verification 16: rejects v0.1.16 published from 0ecd456 behind the 80ecfa candidate", () => {
    const stale = publication(verification16PublishedRelease, { tag: "v0.1.16", commit: verification16Candidate });
    expect(publicationErrors(stale)).toEqual([
      `Latest release commit ${verification16PublishedRelease} does not match site commit ${verification16Candidate}.`,
      `latest.json commit ${verification16PublishedRelease} does not match site commit ${verification16Candidate}.`,
    ]);
  });
  ...
  it("rejects a package omitted from the manifest or checksums", () => {
    ...
    expect(publicationErrors(incomplete)).toEqual([
      "latest.json does not contain the published URL for Local.Live.Captions_0.1.19_amd64.AppImage.",
      "SHA256SUMS does not cover Local.Live.Captions_0.1.19_amd64.AppImage.",
    ]);
  });
});
