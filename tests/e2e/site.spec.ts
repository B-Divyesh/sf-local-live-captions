...
test("download refuses a stale release and selects the exact site build", async ({ page }) => {
  ...
  const assets = [
    { name: "Local.Live.Captions_0.1.15_amd64.AppImage", browser_download_url: "https://github.com/B-Divyesh/sf-local-live-captions/releases/download/v0.1.15/Local.Live.Captions_0.1.15_amd64.AppImage" },
    { name: "Local.Live.Captions_0.1.15_x64_en-US.msi", browser_download_url: "https://github.com/B-Divyesh/sf-local-live-captions/releases/download/v0.1.15/Local.Live.Captions_0.1.15_x64_en-US.msi" },
  ];
  ...
  release = { tag_name: "v0.1.15", target_commitish: "older-sha", assets };
  await page.reload();
  await expect(page.getByText("Downloads are being published.")).toBeVisible();
  await expect(page.getByRole("link", { name: /^Download for/ })).toHaveCount(0);

  release = { tag_name: "v0.1.15", target_commitish: buildSha, assets };
  await page.reload();
  await expect(page.getByRole("link", { name: /^Download for/ })).toHaveAttribute("href", /\/releases\/download\/v0\.1\.15\//);
});
