...
Desktop installers are built only in GitHub Actions. Tag the exact commit with the version from `package.json`, such as `v0.1.10`. To rebuild, run the workflow for that tag. The workflow rejects a tag or commit mismatch. It creates unsigned packages for macOS, Windows, and Linux. It also publishes `SHA256SUMS` and `latest.json` with the source commit. The workflow audits their source identity, package list, URLs, and checksums after publication. After deploying the tagged site build, run `npm run verify:published-release` to check the live identity against GitHub. The site offers packages only when the release and deployed site use the same tag and commit.
